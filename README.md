...
The structure for the residual dynamic modelling inference can be visulized with the above picture. Learn more on how to start the residual inference on the Python side in [this repo](https://github.com/SAABmarine-MEX/saabmarineMEX_learning/tree/svgp/residual_modelling). On the Unity side, make sure you set `useResModel` to true in `BlueROV.cs` (see the folder structure below to find this file) to be able to connect to the Python server.
