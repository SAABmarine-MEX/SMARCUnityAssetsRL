...
namespace DefaultNamespace.BlueROV2.SITL
{
    public class ArduSub : MonoBehaviour
    {
        ...
        public float[] SITL(float[] dofInput)
        {
            /*
             * Input:
             * dofInput - [roll, pitch, throttle (up/down), yaw, forward, lateral] (each between [1100, 1900]), WRONG
             * RIGHT: * dofInput - [pitch, roll, throttle (up/down), yaw, forward, lateral] (each between [1100, 1900]), TODO: change so the input here is like this but need to double check how ardusub actually does it. KOLLA T MATRISEN!!
